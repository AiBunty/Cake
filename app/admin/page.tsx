...
export default function AdminPage() {
  ...
  const loadProducts = async () => {
    const response = await adminListProducts(adminKey);
  ...
  const loadOrders = async () => {
    const response = await adminListOrders(adminKey, 300);
    if (!response.ok || !response.data) {
  ...
  const loadSettings = async () => {
    const response = await adminListSettings(adminKey);
    if (!response.ok || !response.data) {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setError(msg);
      if (msg.toLowerCase().includes("admin key") || msg.toLowerCase().includes("unauthorized")) {
  ...
  const saveProduct = async () => {
    ...
    setLoading(true);
    try {
      const response = await adminUpsertProduct(adminKey, productForm);
      if (!response.ok) {
  ...
  const removeProduct = async (id: string) => {
    ...
    setLoading(true);
    try {
      const response = await adminDeleteProduct(adminKey, id);
      if (!response.ok) {
  ...
  const verifyUpi = async (orderId: string) => {
    ...
    setLoading(true);
    try {
      const response = await markUpiVerified(orderId);
      if (!response.ok) {
  ...
  const saveSetting = async (key: string, value: string) => {
    ...
    setLoading(true);
    try {
      const response = await adminUpsertSetting(adminKey, key, value);
      if (!response.ok) {
        ...
        <Card className="p-4 sm:p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <Input
              ...
              placeholder="Enter admin panel key"
            />
            ...
            <Card className="p-4 sm:p-6 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Input
                  label="Price"
                  type="number"
                  value={String(productForm.price_inr || 0)}
                  onChange={(e) => setProductForm((p) => ({ ...p, price_inr: Number(e.target.value) || 0 }))}
                />
                <Input
                  label="Sort Order"
                  type="number"
                  value={String(productForm.sort_order || 0)}
                  onChange={(e) => setProductForm((p) => ({ ...p, sort_order: Number(e.target.value) || 0 }))}
                />
              </div>
              ...
              <Select
                ...
                onChange={(e) => setProductForm((p) => ({ ...p, is_available: e.target.value === "true" }))}
              >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted">
                        {order.customer_name} • {formatCurrency(order.subtotal_inr)} • {order.payment_method || "RAZORPAY"}
                      </p>
                      <p className="text-xs text-muted">
                        payment_status: {order.payment_status} {order.upi_status ? `• upi_status: ${order.upi_status}` : ""}
                      </p>
                    ...
                    {order.payment_method === "UPI" && order.payment_status !== "PAID" && (
                      <Button
                        ...
                        onClick={() => verifyUpi(order.order_id)}
                        isLoading={loading}
                        disabled={loading}
                      >
                        Verify UPI
